    ...
    >
        <template #container="{ message, acceptCallback, rejectCallback }">
            ...
            <div class="pt-0 px-5 pb-5 flex justify-end gap-2">
                <SecondaryButton @click="rejectCallback" :label="message.rejectProps?.label || '取消'" size="small" />
                <Button @click="acceptCallback" :label="message.acceptProps?.label || '确认'" size="small" />
            </div>
        </template>
    </ConfirmDialog>
</template>
