...
export class UCClient {
  ...
  constructor(config: UCCloudDriveConfig) {
  ...
  private async makeRequest(
    ...
    if (urlOrEndpoint.startsWith('http://') || urlOrEndpoint.startsWith('https://')) {
      ...
      parsedURL.searchParams.set('pr', 'UCBrowser')
    ...
    const defaultHeaders: Record<string, string> = {
      ...
      'Pragma': 'no-cache',
      'Priority': 'u=1, i',
      'Referer': 'https://drive.uc.cn/list',
  ...
  async waitForTaskComplete(taskID: string): Promise<string> {
    ...
    for (let i = 0; i < maxRetries; i++) {
      ...
      console.log('[UC] Task status response:', JSON.stringify(respMap, null, 2))
  ...
  async getShareLink(shareID: string): Promise<{ share_url: string; pwd_id: string; passcode: string; expires_at: number }> {
    console.log('[UC] Calling getShareLink with shareID:', shareID)
    ...
    console.log('[UC] Share password response:', JSON.stringify(respMap, null, 2))
  ...
  async createShare(
    ...
    console.log('[UC] Share response:', JSON.stringify(respMap, null, 2))
    ...
    if (!shareID && !taskSync && taskID) {
      console.log('[UC] Task not synced, waiting for completion...')
    ...
    try {
      ...
      console.log('[UC] Failed to get share link, falling back to manual construction:', error)
      const shareUrl = shareID ? `https://drive.uc.cn/s/${shareID}` : ''
      return {
        success: true,
        code: 'OK',
        message: '分享成功',
